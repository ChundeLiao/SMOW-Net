...
from compare.vmamba import VSSM, LayerNorm2d, VSSBlock, Permute
...
class ChangeDecoder(nn.Module):
    ...
    def forward(self, pre_features, post_features):
        ...
        # ct_tensor_42 = torch.empty(B, C, H, 2 * W).to(self.device)
        ...
        # ct_tensor_43 = torch.empty(B, C, H, 2 * W).to(self.device)
        ...
        # Create an empty tensor of the correct shape (B, C, H, 2*W)
        # ct_tensor_32 = torch.empty(B, C, H, 2 * W).to(self.device)
        ...
        # ct_tensor_33 = torch.empty(B, C, H, 2 * W).to(self.device)
        ...
        # Create an empty tensor of the correct shape (B, C, H, 2*W)
        # ct_tensor_22 = torch.empty(B, C, H, 2 * W).to(self.device)
        ...
        # ct_tensor_23 = torch.empty(B, C, H, 2 * W).to(self.device)
        ...
        # Create an empty tensor of the correct shape (B, C, H, 2*W)
        # ct_tensor_12 = torch.empty(B, C, H, 2 * W).to(self.device)
        ...
        # ct_tensor_13 = torch.empty(B, C, H, 2 * W).to(self.device)
